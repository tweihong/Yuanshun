...
public class CacheManager {
	...
	public <T> String getGroupPath(Class<T> clazz, String group) {
		...
		return classPath + KEY_GROUP_ + StringUtil.getTrimedString(group);
	}
	...
	private SharedPreferences getSharedPreferences(String path) {
		...
		return context.getSharedPreferences(StringUtil.getTrimedString(path), Context.MODE_PRIVATE);
	}
	...
	public <T> List<String> getIdList(Class<T> clazz, String group) {
		SharedPreferences sp = getSharedPreferences(
				getClassPath(clazz) + KEY_GROUP_ + StringUtil.getTrimedString(group));
		String jsonString = sp.getString(KEY_ID_LIST, null);
		if(StringUtil.isNotEmpty(jsonString, true)) {
